...
1. 「**次へ:監視 >**」 をクリックします。 「**Web アプリ**」 ブレードの 「**監視**」 タブで、「**Application Insight の有効化**」 スイッチを 「**いいえ**」 に設定し、「**確認および作成**」 をクリックして、「**作成**」 をクリックします。 
...
1. 「**App Service ビルド サービス**」 を選択し、「**続行**」 をクリックしてから、「**完了**」 をクリックします。 

1. 結果として表示される **Git clone URL**をメモ帳にコピーします。 
...
1. 認証を求められた場合は、`[deployment_user_name]` および対応するパスワード (**P@55w0rd1234**) を入力します。
...
1. 既定のオプションの **「メトリックに基づくスケール」** を選択したまま、**「+ 規則の追加」** をクリックします
...
1. **「追加」** をクリックし、スケール アウト (App Service のプラン) ブレードに戻って、次の設定を指定します (その他は既定値のままにします)。   
