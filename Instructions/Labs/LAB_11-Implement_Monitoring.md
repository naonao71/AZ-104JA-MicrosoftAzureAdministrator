...
1. **「メトリック」** ブレードの既定のグラフでは、使用可能な **メトリック名前空間**は **仮想マシン ホスト**のみです。
...
1. **「az104-11-vm0」** の **「診断設定」** ブレードで、**「ゲスト レベルの監視を有効にする」** をクリックします。
...
1. **「メトリック」** ドロップダウン リストで **「¥Memory¥Avairable Bytes」** を選択し、**「集計」** ドロップダウン リスト で**「最大」** を選択して、結果のグラフを確認します。 
