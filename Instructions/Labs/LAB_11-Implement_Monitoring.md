...
1. Cloud Shell ペインのツール バーで **Upload/Download files**アイコンをクリックし、ドロップダウン メニューで**Upload** をクリックし、**\Allfiles\Labs\11\az104-11-vm-template.json** と **\Allfiles\Labs\11\az104-11-vm-parameters.json**をCloud Shell ホーム ディレクトリにアップロードします。
...
1. 「**Log Analytics ワークスペースの作成**」 ブレードで、 次の設定を指定して**確認および作成**をクリックし**作成**をクリックします。
...
1. **Az104-11-vm0** ブレードの**監視**セクションで、**ログ**をクリックして、**Enable** (有効にする) をクリックします。  
...
1. ブレードの右側にあるグラフ ウィンドウの 「**リソース**」 ドロップダウン リストで、「**+ リソースの選択**」 をクリックします。   
...
1. 「**モニター - メトリック**」 ブレードで、「**新しいアラートルール**」 をクリックします。
...
1. 「**ルールの作成**」 ブレードの 「**条件**」 セクションで、「**percentage cpu の 平均 が <ロジックが定義されていません> % 次の値より大きい 場合**」 をクリック します。 

1. 「**シグナル ロジックの構成**」 ブレードの「**アラート ロジック**」 セクションで次の設定を指定し (他のユーザーには既定値をそのまま使用します)、「**完了**」 をクリックします。
...
1. 「 **ルールの作成** 」 ブレードの 「**アクション グループ (オプション)**」 セクションで、「**アクショングループの選択**」 をクリックし、**＋アクション グループの作成** をクリックします。     

1. 「**アクション グループの作成**」 ブレードで、次の設定を指定します (既定値を他のユーザーのままにします)。
...
1. **[次へ:通知>]**をクリックして「**アクション グループの作成**」 ブレードの 「**通知**」 セクションで、次の設定を指定します (既定値を他のユーザーのままにします)。

    | 設定 | 値 |
    | --- | --- |
    | 通知の種類 | **Email/SMS/Push/Voice** |
    | 名前： | **az104-11-ag1 email** |


1. **az104-11-ag1 email**アクション行で、「**詳細の編集**」  をクリックします。

1. 「**電子メール/SMS/プッシュ/音声**」 ブレードで、「**電子メール**」 チェックボックスをオンにし、「**電子メール**」 テキストボックスに自分のメールアドレスを入力し、他の設定は既定値のままで 「**OK**」をクリックし、「**アクション グループの作成**」 ブレードで 「**確認および作成**」して「**作成**」 をクリックします。

1. 「**ルールの 作成**」 ブレードに戻り、次の設定を指定します (既定値を他のユーザーのままにします)。

    | 設定 | 値 |
    | --- | --- |
    | 警告ルール名 | **Percentage CPUが テストしきい値を超える** |
    | 説明 | **Percentage CPUが テストしきい値を超える** |
    | 重大度 | **Sev 3** |
    | 作成時にルールを有効にする | **Yes** |

1. 「**アラートルールの作成**」 をクリックし、「**ルールの作成**」 ブレードを閉じます 。
...
1. ツールバーの **Example queries** (クエリ例) をクリックして、各タブを確認し、**Virtual machine available memory** (メモリを利用できる仮想マシン)、**実行** の順にクリックします。
