...
1. 「**Metric (メトリック)**」ドロップダウン リストから「**Percentage CPU (CPU 使用率)**」、「**集計**」ドロップダウン リストから「**平均**」をそれぞれ選択し、結果のグラフを確認します。
...
1. 「**アラート ルールの作成**」ブレードで、「**条件**」セクションの既存の条件をクリックします。          

1. 「**シグナル ロジックの構成**」ブレード上のシグナル リスト内の「**アラート ロジック**」セクションで、次の設定を指定し (それ以外の値はデフォルトを使用)、「**完了**」をクリックします。
...
1. 「**ルールの作成**」ブレード上の「**アクション グループ**」セクションで、「**アクション グループの選択**」をクリックし、「**+ アクショングループの作成**」ボタンをクリックします。     
...
1. ツールバーの「**Example queries (クエリの例)**」をクリックし、「**サンプル クエリを使用して作業を開始する**」ペインで各タブを確認し、「**Track VM availability (VM 可用性の追跡)**」を検索して「**実行**」をクリックします。
