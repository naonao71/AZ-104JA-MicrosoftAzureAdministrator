...
1. **「テンプレートの編集」** ブレードの 「テンプレートの内容を表示する」 セクションで、行 **35** `"vmSize": "Standard_D2s_v3"`を次の行に置き換えます)。
...
1. **「テンプレートの編集」** ブレードの 「テンプレートの内容を表示する」 セクションで、行 **55** `"vmSize": 「 」` line) を次のコードに置き換えます。
...
1. 変更を保存して、[**az10408vms0**] ブレードの [**設定**] セクションで [**インスタンス**] をクリックし、仮想マシン スケール セットのインスタンスの横にあるチェックボックスをオンにし、[**アップグレード**] をクリックして、確認を求められたら、[**はい**] を選択します。
