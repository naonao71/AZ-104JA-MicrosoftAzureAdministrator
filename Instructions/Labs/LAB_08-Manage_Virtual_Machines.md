...
#### タスク 4: Microsoft.Insights and Microsoft.AlertsManagement リソースプロバイダーを登録する 
...
1. 「**仮想マシン スケール セットの作成**」 ブレードの 「**詳細設定**」 タブで 、次の設定を指定し (その他は規定値のままにします) 、「**確認および作成**」 をクリックします。

    | 設定 | 値 | 
    | --- | --- |
    | 拡散アルゴリズム | **展開の修復 (ゾーンでは推奨されません)** |
...
1. 「**+ 規則を追加**」 リンクをクリックし、「**スケール ルール**」 ブレードで次の設定を指定します。その他の設定は既定値のままにします。
...
1. **「az10408vms0」** ブレードで 、**「ストレージ」** をクリックし、 **「+ データ ディスクの追加」** をクリックして、新しいマネージド ディスクを次の設定で保存します (その他は既定値のままにします)。     
