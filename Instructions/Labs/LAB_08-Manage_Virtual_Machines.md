...
1. **次へ:詳細設定>** をクリックします。**仮想マシンの作成**ブレードの**詳細設定**タブで、利用可能な設定を確認のみして、**レビューと作成**をクリックします。
...
1. Azure portal で、**ストレージ アカウント** を特定し、**ストレージ アカウント** ブレードで、前のタスクで作成した診断ストレージ アカウントの項目をクリックします。 
...
1. **新しいコンテナー** ブレードで、次の設定を指定して、**作成** をクリックします (その他の値は既定値のまま)。
     ...
     | --- | --- |	 
	 | 名前 | **scripts** |
     | パブリック アクセス レベル | **非公開 (匿名アクセスを許可しない**) |

1. コンテナーの一覧が表示されているストレージ アカウント ブレードに戻り、**scripts** をクリックします。

1. **Scripts** ブレードで、**アップロード**をクリックします。

1. **BLOB のアプロード** ブレードで、フォルダー アイコンをクリックし、**開く**ダイアログ ボックスで、**\\Allfiles\\Labs\\08** フォルダーに移動し、**az104-08-install_IIS.ps1** を選択してから、**開く**をクリックして **BLOB のアップロード** ブレードに戻り、**アップロード**をクリックします。 
...
1. **ストレージ アカウント** ブレードで、**az104-08-install_IIS.ps1** スクリプトをアップロードしたストレージ アカウントの名前をクリックし、**コンテナー** ブレードで**スクリプト**をクリックします。**スクリプト** ブレードで、**az104-08-install_IIS.ps1** をクリックしてから**選択**をクリックします。 
...
1. **Az10408vmss0** ブレードで、「**ディスク**」、「**+ データ ディスクの追加**」の順にクリックし、新しい管理対象ディスクを次の設定に従って添付します　(それ以外の値はデフォルトを使用)。     
