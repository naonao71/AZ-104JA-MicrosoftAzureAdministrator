...
1. 次の設定でピアリングを追加し (他のユーザーには既定値を残します)、「追加」 をクリックします。
    ...
    | 設定 | 値 |
    | --- | --- |
    | このバーチャル ネットワーク: ピアリング リンク名 | **az104-06-vnet01_to_az104-06-vnet2** |
    | リモート バーチャル ネットワークのトラフィック | **許可 (既定)** |
    | リモート バーチャル ネットワークから転送されたトラフィック | **このバーチャル ネットワークの外部から発信されるトラフィックをブロックする** |
    | バーチャル ネットワーク ゲートウェイ | **なし (既定)** |
    | リモート バーチャル ネットワーク: ピアリング リンク名 | **az104-06-vnet2_to_az104-06-vnet01** |
...
1. 次の設定でピアリングを追加し (他のユーザーには既定値を残します)、「追加」 をクリックします。
    ...
    | 設定 | 値 |
    | --- | --- |
    | このバーチャル ネットワーク: ピアリング リンク名 | **az104-06-vnet01_to_az104-06-vnet3** |
    | リモート バーチャル ネットワークのトラフィック | **許可 (既定)** |
    | リモート バーチャル ネットワークから転送されたトラフィック | **このバーチャル ネットワークの外部から発信されるトラフィックをブロックする** |
    | バーチャル ネットワーク ゲートウェイ | **なし (既定)** |
    | リモート バーチャル ネットワーク: ピアリング リンク名 | **az104-06-vnet3_to_az104-06-vnet01** |
    | バーチャル ネットワーク デプロイ モデル | **Resource Manager** |
    | リソース ID を知っている | enabled |
