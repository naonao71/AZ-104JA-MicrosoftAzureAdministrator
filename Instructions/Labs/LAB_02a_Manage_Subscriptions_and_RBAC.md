...
1. **「管理グループ」**ブレードで、**「子サブスクリプション」**列内の、このラボで使用した Azure サブスクリプションの移動先にしたい管理グループの名前が表示されている行で、サブスクリプションの現在の数を示すリンクを選択します。

   >** 注**: このラボを実行する前にカスタム管理グループ階層を作成した場合を除き、ターゲットの管理グループが**テナント ルート管理グループ**であると考えられます。
   
1. ターゲットの管理グループの**サブスクリプション**ブレードで**「+ 追加」**を選択します。

1. **「サブスクリプションの追加」**ブレードの**「サブスクリプション」**ドロップダウンリストから、このラボで使用した Azure サブスクリプションの名前を選択して**「保存」**をクリックします。
