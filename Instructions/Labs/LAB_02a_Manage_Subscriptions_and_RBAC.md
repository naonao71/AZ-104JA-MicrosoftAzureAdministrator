...
1. **InPrivate**ブラウザー ウィンドウの 「 **ヘルプとサポート | 新しいサポートリクエスト**」 ブレードの 「**基本**」 タブで、「 **サービスとサブスクリプションの制限 (クォータ)**」 の問題の種類を選択し、このラボで使用しているサブスクリプションが 「**サブスクリプション**」 ドロップダウン リストに表示されていることを確認します。
