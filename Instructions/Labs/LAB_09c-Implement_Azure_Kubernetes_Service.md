...
1. 「**次へ**」をクリックします 「**ノード プール >**」、および「**Kubernetes クラスターを作成**」ブレードの「**ノード プール**」タブで、次の設定を指定します (それ以外の値はデフォルトを使用)。
...
1. 「**次へ:**」 をクリックする「**ネットワーク >**」 をクリックし、「**Kubernetes クラスターの作成**」 ブレードの 「**ネットワーク**」 タブで次の設定を指定します (その他の設定は既定値のままにします)。
...
1. 「**次へ**」をクリックします「**Integration (統合) >**」、および「**Kubernetes クラスターを作成**」ブレード上の「**Integration (統合)**」タブで、「**コンテナーの監視を有効にする**」を「**無効**」に設定し、「**確認と作成**」、「**作成**」の順にクリックします。 
...
1. 「**Cloud Shell**」 ペインで出力を確認し、このポイントでクラスターを構成する 1 つのノードが **準備完了** の状態を報告していることを確認します。 
...
1. ブラウザー ウィンドウを開いて、前の手順で取得した IP アドレスに接続します。ブラウザー ページに 「**nginx へようこそ!**」 というメッセージが表示されることを確認します。
