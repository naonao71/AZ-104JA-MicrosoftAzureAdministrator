...
# ラボ 02b - Azure ポリシーを介してガバナンスを管理する
# 受講者用ラボ マニュアル
...
1. 「ストレージ アカウント」 ブレードで、ストレージ アカウントを含むリソース グループの名前のリンクをクリックします。
...
1. ストレージ アカウント ブレードに戻ります。「**概要**」 を確認し、新しいタグがストレージ アカウントには継承されていないことを確認します。 
...
1. 「**新規**」 ブレードで、「**ストレージ アカウント**」 を検索して選択し、「**作成**」 をクリック します。 
...
1. 検証が失敗したことを確認します。「**検証に失敗しました。詳細を表示するには、ここをクリックしてください。**」 をクリックして 「**エラー**」 ブレードを表示し、エラーの原因を特定します。 
...
1. 「**次へ**」 をクリックし、「**修復**」 タブで次の設定を構成します。その他の設定は既定値のままにします。
    ...
    | --- | --- |
    | 修復タスクを作成する | (有効) |
    | 修復するポリシー | **存在しない場合、リソース グループからタグを継承する** |
...
1. ストレージ アカウントのリストで、このラボの最後で作成したストレージ アカウントに対応するリソース グループを選択します。「**タグ**」を選択してから、**Role:Infra** タグで「**削除**」 (ごみ箱の右側) をクリックして、「**保存**」を押します。

1. ポータルで、「**ストレージ アカウント**」を再度探して選択するか、メニュー上部から「**ストレージ アカウント**」を選択します。
