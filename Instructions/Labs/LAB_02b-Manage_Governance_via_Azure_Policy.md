...
- Cloud Shell ストレージ アカウントなどストラクチャ リソースのみを含むリソース グループにタグを付ける
...
1. 「**新規**」 ブレードで、「**ストレージ アカウント**」 を検索して選択し、「**作成**」 をクリックします。 
...
1. 検証が失敗したことに注意してください。**「検証に失敗しました」** リンクをクリックします。**「詳細情報を表示するには、ここをクリックします」** をクリックして 「**エラー**」 ブレードを表示し、エラーの原因を特定します。 
...
1. 割り当てのリストで、**「Infra 値を持つ Role タグを必要とする」** ポリシーの割り当てを表す行の 「省略記号」 アイコンを右クリックし、**「割り当ての削除」** メニューの項目を使用してこの割り当てを削除します。 
