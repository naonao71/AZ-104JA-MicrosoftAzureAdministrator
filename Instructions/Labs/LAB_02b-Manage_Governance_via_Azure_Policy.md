...
1. ストレージ アカウント ブレードに戻ります。[**概要**] をレビューし、新しいタグがストレージ アカウントに自動的に割り当てられていないことに注意してください。 
...
1. 「**リソースでタグとその値が必要**」という組み込みポリシーのエントリーをクリックし、その定義をレビューします。

1. [**リソースでタグとその値が必要**] という組み込みポリシーの定義ブレードで、[**割り当て**] をクリックします。

1. 省略ボタンをクリックして次の値を選択し、**スコープ**を指定します。
...
1. 次の設定を指定して、割り当ての **基本**プロパティを構成します (その他は既定値のままにします)。 
    ...
    | 説明 | **Cloud Shell リソース グループ内のすべてのリソースの Infra 値を含むロール タグを必要とします**|
    | ポリシーの適用 | 有効にする |
...
1. [**次へ**] をクリックして、[**修復**] タブをレビューします。[**管理されたIDを作成する**] のチェックを外したままにしておきます。 
...
1. [**ポリシー定義**] を指定するには、省略記号ボタンをクリックし、[**存在しない場合は、リソース グループからタグを継承する**] を検索して選択します。

1. 次の設定を指定して、その割り当ての残りの [**基本**] プロパティを構成します。その他の設定は既定値のままにします。

    | 設定 | 値 |
    | --- | --- |
    | 割り当て名 | **ロール タグとその Infra 値がなければ、Cloud Shell リソース グループから継承する**|
    | 説明 | **ロール タグとその Infra 値がなければ、Cloud Shell リソース グループから継承する**|
    | ポリシーの適用 | Enabled |
