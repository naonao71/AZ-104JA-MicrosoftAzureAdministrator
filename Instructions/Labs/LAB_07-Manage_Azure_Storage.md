...
1. 「**ライセンス**」 ブレードで、使用可能なオプションを確認します。 
    ...
    > **注意**: 次のオプションがあります。BLOB のダウンロード、アクセス階層の変更 (通常は **ホット** に設定)、リースの取得 (リース状態は現在設定されている**ロック解除**状態から**ロック済み**状態に変更されます)、BLOB が変更または削除されないように保護、カスタム メタデータの割り当て (任意のキーおよび値ペア)。また、ファイルをダウンロードせずに、Azure portal インターフェイスで直接**編集**することもできます。スナップショットを作成し、SAS トークンを生成することもできます (次のタスクでこのオプションを確認します)。 
